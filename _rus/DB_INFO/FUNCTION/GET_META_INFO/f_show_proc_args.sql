...
-- 2019-03-01 Nick.  Ничего особенного, просматриваю код.
-- ====================================================================================================================
/* --------------------------------------------------------------------------------------------------------------------
	Входные параметры:
...
DROP FUNCTION IF EXISTS db_info.f_show_proc_args(oid);
CREATE OR REPLACE FUNCTION db_info.f_show_proc_args
(
        p_proc_oid  oid -- Идентификатор объекта процедуры
)
RETURNS TABLE
(
...
$$
  BEGIN
    IF EXISTS (
            SELECT pcpp.oid FROM pg_catalog.pg_proc pcpp WHERE pcpp.oid = p_proc_oid
    )
    THEN
            RETURN QUERY
            WITH nums AS (
            ...
            SELECT
                   ...
                   ,substring (
                               (SELECT description FROM pg_catalog.pg_description WHERE objoid = p_proc_oid)
                               ,argname || '[^\r\n]*[-]{2}[\s]*([^\r\n]*)[\s]*[\r\n]?'::text
                    )::public.t_description AS arginfo
            FROM nums fin;
    ELSE
            RAISE NOTICE 'Процедура % не существует.', p_proc_oid;
            RETURN;
    END IF;
  END;
$$
...
COMMENT ON FUNCTION db_info.f_show_proc_args (oid)
IS '24dbg/5830/229: Получение описания входных и выходных аргументов процедуры.
...
-- SELECT format_type('small_t'::regtype, 2)
-- SELECT * FROM com.com_f_domain_prop_s('t_money');
-- SELECT info.* FROM pg_catalog.pg_proc pcpp, db_info.f_show_proc_args (pcpp.oid) info WHERE pcpp.proname = 'com_p_notify_put_1';
-- SELECT info.* FROM pg_catalog.pg_proc pcpp, db_info.f_show_proc_args (pcpp.oid) info WHERE pcpp.proname = 'pmt_f_payment_pl_qty_get';
-- SELECT * FROM pg_type WHERE typname LIKE '%float%' 't_money'
