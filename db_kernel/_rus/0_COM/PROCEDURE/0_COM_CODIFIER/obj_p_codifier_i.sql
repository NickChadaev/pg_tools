/*
      Аргументы:
                 p_paren t      id_t       -- ID родительского идентификатора      
          		 ,p_code         t_str60    -- Код                                                          
                ...
                -- 
   	Выходные величины: 
		 result_t.rc > 0 - процедура завершилась успешно, ID созданной записи
		 result_t.rc -1  - процедура завершилась с ошибкой, сообщение об ошибке

	Особенности
				Для генерации ИД PK используется последовательность com.obj_codifier_codif_id_seq
---------------------------------------------------------------------------------------------*/
DROP FUNCTION IF EXISTS com_codifier.obj_p_codifier_i (public.id_t, public.t_str60, public.t_str250, public.t_guid, public.t_code1, public.t_timestamp, public.t_timestamp);
...
$$
    ...
    -- 2015-07-20 Gregory добавлено поле журнала учета изменений
    -- -------------------------------------------------------------------------------------
...
DECLARE 
 ...
 BEGIN
       IF ((_code IS NULL) OR (_name IS NULL) OR (p_scode IS NULL) OR 
           (p_parent IS NULL) OR (p_codif_uuid IS NULL) OR (p_date_from IS NULL) OR
           (p_date_to IS NULL)
          )
        THEN
       ...
       END IF;       
       --       
       INSERT INTO com.obj_codifier (  parent_codif_id
                                      ,small_code
                                      ,codif_code
                                      ,codif_name
                                      ,codif_uuid   
...
COMMENT ON FUNCTION com_codifier.obj_p_codifier_i (public.id_t, public.t_str60, public.t_str250, public.t_guid, public.t_code1, public.t_timestamp, public.t_timestamp) 
   IS '165: Добавление новой записи в кодификатор, ID родителя, 

    Аргументы:
         p_paren t      public.id_t            -- ID родительского идентификатора      
        ,p_code         public.t_str60         -- Код                                                          
        ,p_name         public.t_str250        -- Наименование                                        
        ,p_codif_uuid   public.t_guid          -- UUID       
         --  Далее значения по умолчанию
...
-- -------------------------------------------------------------------------------
--  SELECT * FROM com_codifier.obj_p_codifier_i ( 2, 'C_NSO_CLASS', 'Классификатор', 'C' );
--  SELECT * FROM com_codifier.obj_p_codifier_i ( 35, 'C_NSO_TTH',   'ТТХ', NEWID() ); -- Значение по умолчанию
--  SELECT * FROM com_codifier.obj_p_codifier_i ( 2234, 'C_NSO_CLASS111', 'Классификатор111', 'C' );-- Нет такого родителя
--  SELECT * FROM com_codifier.obj_p_codifier_i ( 2, 'C_NSO_CLASS2', 'Классификатор2', NEWID() );
--  SELECT * FROM com_codifier.obj_p_codifier_i ( null, null, 'Классификатор2');
