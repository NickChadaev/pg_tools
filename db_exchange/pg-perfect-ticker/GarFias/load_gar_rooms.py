...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_rooms (
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_rooms: "

# ------------------------------------------------------------------------------

class RoomsHandler( xml.sax.ContentHandler ):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "ROOM":
         ...
         self.room_number  = attributes.get("NUMBER",'')
         self.room_type    = attributes.get("ROOMTYPE",'')
         ...
         #
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.object_id, self.object_guid, self.change_id,\
                  self.room_number, self.room_type, self.oper_type_id, self.prev_id, self.next_id,\
                  self.update_date, self.start_date, self.end_date, self.is_actual, self.is_active))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = RoomsHandler (self.conn_x, self.cur_x, 0)
