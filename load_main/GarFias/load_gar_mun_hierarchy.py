...
# FILE: load_gar_mun_hierarchy.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_mun_hierarchy (
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_mun_hierarchy: "

# --------------------------------------------------------------------------

class MunHierarchyHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "ITEM":
         ...
         self.oktmo         = attributes.get("OKTMO",'')
         ...
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.object_id, self.parent_obj_id,\
                  self.change_id, self.oktmo, self.prev_id, self.next_id, self.update_date,\
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = MunHierarchyHandler (self.conn_x, self.cur_x, 0)
