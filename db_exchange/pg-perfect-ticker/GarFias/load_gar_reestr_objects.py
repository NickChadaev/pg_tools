...
# FILE: load_gar_reestr_objects.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_reestr_objects (
                   ...
                   ,i_create_date := (NULLIF (%s, ''))::date
                   ,i_update_date := (NULLIF (%s, ''))::date
);
"""
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_reestr_objects: "

# --------------------------------------------------------------------------

class ReestrObjectsHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "OBJECT":
         ...
         self.level_id    = attributes.get("LEVELID",'')
         self.create_date = attributes.get("CREATEDATE",'')
         self.update_date = attributes.get("UPDATEDATE",'')
         
         try:
              self.cur_x.execute (CALL_PROC, (self.object_id, self.object_guid,\
                  self.change_id, self.is_active, self.level_id, self.create_date,\
                      self.update_date))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = ReestrObjectsHandler (self.conn_x, self.cur_x, 0)
