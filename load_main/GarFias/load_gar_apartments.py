...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_apartments (
                    i_id           := (NULLIF (%s, ''))::bigint
                   ,i_object_id    := (NULLIF (%s, ''))::bigint
                   ,i_object_guid  := (NULLIF (%s, ''))::uuid
                   ,i_change_id    := (NULLIF (%s, ''))::bigint
                   ,i_apart_number := (NULLIF (%s, ''))::varchar(50)
                   ,i_apart_type   := (NULLIF (%s, ''))::bigint
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_apartments: "

# ------------------------------------------------------------------------------

class ApartmentsHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "APARTMENT":
         ...
         self.apart_number = attributes.get("NUMBER",'')
         self.apart_type   = attributes.get("APARTTYPE",'')
         ...
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.object_id, self.object_guid,\
                  self.change_id, self.apart_number, self.apart_type, self.oper_type_id,\
                      self.prev_id, self.next_id, self.update_date, self.start_date,\
                          self.end_date, self.is_actual, self.is_active))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = ApartmentsHandler (self.conn_x, self.cur_x, 0)
