DROP FUNCTION IF EXISTS fsc_orange_pcg.fsc_order_corr_crt (
      text, text, text, json, text, text, text, text
);      
CREATE OR REPLACE FUNCTION fsc_orange_pcg.fsc_order_corr_crt (
...
AS $$
   -- =============================================================================== --
   --  2023-08-29  Создание объекта "order-коррекция чека" (Данные для POST-запроса)  --
   -- =============================================================================== --
   ...
   DECLARE
    ...
    JKEYS CONSTANT text[] := array [ 'id',          'inn',            'group', 'content',           'key'
                                   , 'callbackUrl', 'callbackApiKey', 'meta'  
                                   ]::text[];  
     --                                 6             7                  8        9           
...
COMMENT ON FUNCTION fsc_orange_pcg.fsc_order_corr_crt (text, text, text, json, text, text, text, text)     
    IS 'Создание объекта "order-коррекция чека" (Данные для POST-запроса) ORANGE';	
--
--  USE CASE:
--
