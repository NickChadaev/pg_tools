...
IA = 7     
 ...
 def __init__(self, p_host_ip, p_port, p_db_name, p_user_name, p_yaml_path, p_yaml_file\
     ,p_id_region, p_data_version, p_std_out, p_std_err, p_prt_sw = True, p_proc_mark = EMP, p_log_mark = EMP,\
         p_fserver_nmb = None):
     ...
     self.file_path = self.file_path + p_data_version.replace (bMN, bEMP)
     #------------------------------------------------------------
...
if __name__ == '__main__':
    try:
        ...
        aaup = AdrUpload ( sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4],\
            sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], bOUT_NAME, bERR_NAME) 
