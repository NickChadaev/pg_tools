...
ALTER TABLE dict.dct_tp_client
  OWNER TO mb_owner;
...
ALTER TABLE dict.dct_tp_contact
  OWNER TO mb_owner;
...
IS 'Реестр системных сущностей представляет собой список таблиц, которые НЕ ведутся с помощью метамодели данных Смородины-Диалог';
...
ALTER TABLE dict.dct_sys_entity
  OWNER TO mb_owner;
...
ALTER TABLE dict.dct_crm_service
  OWNER TO mb_owner;
...
ALTER TABLE dict.dct_service_status
  OWNER TO mb_owner;
...
IS 'Параметры для особых ситуаций. Таблица используется для дополненительных параметров выгрузки (например исключение определенных id из выгрузок)';
...
ALTER TABLE dict.dct_spec_params
  OWNER TO mb_owner;
...
ALTER TABLE dict.dct_step_service
  OWNER TO mb_owner;
