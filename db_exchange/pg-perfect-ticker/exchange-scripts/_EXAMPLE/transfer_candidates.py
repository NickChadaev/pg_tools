...
FETCH_EVENTS_SQL = '''\
delete from face_control_transfer_var.queue q
        where q.queue_item_id = any (
            select qq.queue_item_id
                    from face_control_transfer_var.queue qq
                    order by qq.queue_item_id
                    limit %(fetch_count)s
                    for update
        )
        returning q.tf_check::text\
'''

HANDLE_EVENT_SQL = '''\
select face_control_traffic.handle_tf_check (%s)\
'''
