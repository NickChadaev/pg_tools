DROP FUNCTION IF EXISTS auth_serv_obj.auth_f_role_s (public.t_sysname);
CREATE OR REPLACE FUNCTION auth_serv_obj.auth_f_role_s (
    p_role_name         public.t_sysname    -- Имя роли
)
RETURNS TABLE  (
...
$$
    ...
    /* Created on:     02.09.2015 11:00:00   Функция-перегрузка.                                */
    /* ---------------------------------------------------------------------------------------- */
    /*  Особенности:                                                                            */
...
BEGIN
    RETURN QUERY(
        WITH RECURSIVE role_info AS (
            ...
            WHERE ( COALESCE ( p_role_name, auth_role.role_name ) = auth_role.role_name )
...
COMMENT ON FUNCTION auth_serv_obj.auth_f_role_s( public.t_sysname ) 
    IS '186: Отображение свойств существующей роли. Аргумент Имя роли.
Входные параметры:
...
Выходные параметры:
    ...
    -- --------------------------------------------------------------------------------
-- SELECT * FROM plpgsql_check_function_tb ('auth_serv_obj.auth_f_role_s (public.t_sysname) ', security_warnings := true, fatal_errors := false);
-- OK
-- SELECT * from auth_serv_obj.auth_f_role_s('xxx'::t_sysname);

-- SELECT * from auth_serv_obj.auth_f_role_s(NULL::id_t);
