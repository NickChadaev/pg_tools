...
DROP FUNCTION IF EXISTS com_domain.nso_p_domain_column_d ( public.t_str60 );
CREATE OR REPLACE FUNCTION com_domain.nso_p_domain_column_d (
	          p_attr_code  public.t_str60 -- ID атрибута
)
RETURNS public.result_long_t 
  ...
  DECLARE
    ...
    _attr_code  public.t_str60 := utl.com_f_empty_string_to_null (upper(btrim (p_attr_code)));
  ...
  BEGIN
  	...
  	DELETE FROM ONLY com.nso_domain_column WHERE (attr_code = _attr_code) RETURNING attr_id INTO _attr_id;
  	
  	IF FOUND
  	THEN
       	RETURN (_attr_id::bigint, 'Успешно удален атрибут'::text )::public.result_long_t;
  	ELSE
        _err_args [1] := _attr_code::text;
...
COMMENT ON FUNCTION com_domain.nso_p_domain_column_d (public.t_str60) 
     IS '194: Удаление записи в домене атрибутов. Агрумент код атрибута
     
	Аргументы:
		1) p_attr_code	  public.t_str60	-- Идентификатор атрибута
