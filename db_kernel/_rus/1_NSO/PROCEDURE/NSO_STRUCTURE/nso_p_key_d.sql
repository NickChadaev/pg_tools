...
-------------------------------------------------------------------------------------------------------------------- */
DROP FUNCTION IF EXISTS nso_structure.nso_p_key_d ( public.id_t );
CREATE OR REPLACE FUNCTION nso_structure.nso_p_key_d (
        p_key_id public.id_t -- Код ключа
)
...
AS
 $$
    ...
    --    2019-01-10 Функция-перегрузка
    -- ====================================================================== 
  DECLARE
    _key_id  public.id_t := p_key_id;
    _nso_id  public.id_t;
    _s_code  public.t_code1;
    _col_id  public.t_arr_id;
    
    _key_code public.t_str60;   -- Код ключа/марекера ключа
  ...
  BEGIN
	...
	THEN
        ...
        ELSE
                SELECT key_code, nso_id, key_small_code  INTO _key_code, _nso_id, _s_code
                    FROM ONLY nso.nso_key WHERE key_id = _key_id;
	END IF;
...
COMMENT ON FUNCTION nso_structure.nso_p_key_d (public.id_t) IS '256: Удаление ключа по коду.
	Входные параметры:
		1) p_key_id public.id_t -- ID ключа
