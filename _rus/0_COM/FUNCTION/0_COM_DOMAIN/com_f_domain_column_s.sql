DROP FUNCTION IF EXISTS com_domain.nso_f_domain_column_s ( public.id_t );
CREATE OR REPLACE FUNCTION com_domain.nso_f_domain_column_s ( p_attr_id public.id_t = NULL ) 
 RETURNS TABLE  (
 ...
 SET search_path=com_domain,com,nso,PUBLIC
  AS
   $$
      ...
      -- «JOIN ONLY nso.nso_object» и  «FROM ONLY com.nso_domain_column»                                   --
         ...
         WITH RECURSIVE cd (    
               ...
               ,id_log            -- Идентификатор журнала
                  ...
                  SELECT  
                     ...
                     ,d.date_from         
                     ,d.date_to           
                     ,CAST ( ARRAY [d.attr_id::int8] AS public.t_arr_id) -- Nick 2019-07-03
                     ,1::public.small_t
                  ...
                  WHERE (( d.parent_attr_id IS NULL ) AND ( p_attr_id IS NULL )) OR
                        (( d.attr_id = p_attr_id ) AND ( p_attr_id IS NOT NULL)) 
                  ...
                  UNION ALL
                   ...
                   SELECT 
                         ...
                         ,d.date_from         
                         ,d.date_to           
                         ,CAST (  x.tree_d::int8[] || d.attr_id::int8 AS public.t_arr_id ) -- Nick 2019-07-03
...
COMMENT ON FUNCTION com_domain.nso_f_domain_column_s ( public.id_t ) 
   IS '111: Список атрибутов, развёрнут по уровням иерархии, аргумент ID атрибута.
...
';
-- -------------------------------------------------------------------------------------------
-- SELECT * FROM plpgsql_check_function_tb ('com_domain.nso_f_domain_column_s (public.id_t)');
-- -------------------------------------------------------------------------------------------	
--   17|'RETURN QUERY'|'42883'|'operator does not exist: t_arr_id || id_t'|''|'No operator matches the given name and argument type(s). 
--                                                                             You might need to add explicit type casts.'
-- SELECT * FROM com_domain.nso_f_domain_column_s ();
-- SELECT * FROM com_domain.nso_f_domain_column_s (22);
-- SELECT * FROM com.nso_domain_column; 
-- Суммарное время выполнения запроса: 200 ms.
-- 869 строк получено.
--
-- SELECT * FROM com.nso_f_domain_column_s (33); -- 'Атрибуты для заполнения классификаторов'
