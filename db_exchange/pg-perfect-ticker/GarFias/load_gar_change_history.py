...
# FILE: load_gar_change_history.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_change_history (
                    i_change_id     := (NULLIF (%s, ''))::bigint
                   ,i_object_id     := (NULLIF (%s, ''))::bigint
                   ,i_adr_object_id := (NULLIF (%s, ''))::uuid
                   ,i_oper_type_id  := (NULLIF (%s, ''))::bigint
                   ,i_ndoc_id       := (NULLIF (%s, ''))::bigint
                   ,i_change_date   := (NULLIF (%s, ''))::date
);
"""
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_change_history: "

# --------------------------------------------------------------------------

class ChangeHistoryHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "ITEM":
         ...
         self.adr_object_id = attributes.get("ADROBJECTID",'')     
         self.oper_type_id  = attributes.get("OPERTYPEID",'')        
         self.ndoc_id       = attributes.get("NDOCID",'')     
         self.change_date   = attributes.get("CHANGEDATE",'')      
         
         try:
              self.cur_x.execute (CALL_PROC, (self.change_id, self.object_id, self.adr_object_id,\
                  self.oper_type_id, self.ndoc_id, self.change_date))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    rc = -1      
    Handler = ChangeHistoryHandler (self.conn_x, self.cur_x, 0)
