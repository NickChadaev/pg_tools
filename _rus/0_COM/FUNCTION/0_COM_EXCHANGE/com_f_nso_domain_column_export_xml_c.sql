/* --------------------------------------------------------------------------------------------------------------------
	Входные параметры:
		1) p_attr_code    public.t_str60       -- Код
...
DROP FUNCTION IF EXISTS com_exchange.com_f_nso_domain_column_export_xml(public.t_str60,public.t_boolean,public.t_sysname,public.t_timestamp,public.t_timestamp);
CREATE OR REPLACE FUNCTION com_exchange.com_f_nso_domain_column_export_xml
(
        p_attr_code    public.t_str60                    -- Код
...
AS 
 $$
    ...
    -- 2016-05-16  Nick  Функция-перегрузка, аргумент Код атрибута                                                --
   ...
   DECLARE
        ...
        _attr_code  public.t_str60;     -- Код атрибута Nick 2016-06-29
   ...
   BEGIN
        ...
        _attr_code := utl.com_f_empty_string_to_null(btrim (upper(p_attr_code))); -- Nick 2016-06-29
        IF NOT EXISTS ( SELECT 1 FROM ONLY com.nso_domain_column -- 2016-05-16 Nick забыто ONLY
                         WHERE attr_code = _attr_code
        )
        THEN
               _err_args [1] := _attr_code;
               RAISE SQLSTATE '61074'; -- Запись не найдена  не найдена 
        END IF;
...
COMMENT ON FUNCTION com_exchange.com_f_nso_domain_column_export_xml(public.t_str60,public.t_boolean,public.t_sysname,public.t_timestamp,public.t_timestamp)
IS '162: Экспорт домена колонки в XML.
	...
	Выходные параметры:
		1) _result        public.result_long_t -- Результирующий контент
        Особенности:
                если p_date_from равен p_date_to режим выгрузки состояния на дату
                если p_date_from не равен p_date_to режим выгрузки изменений';
-- ------------------------------------------------------------------------------------------------------------- --
-- SELECT * FROM com.nso_f_domain_column_s (); -- Все колонки (атрибуты).                                -- 74 rows
-- SELECT * FROM com.nso_f_domain_column_s ('APP_NODE'); -- Прикладные колонки (атрибуты)                -- 29 rows
-- SELECT * FROM com.nso_f_domain_column_s ('TECH_NODE'); -- Все технические колонки                     -- 44 rows
