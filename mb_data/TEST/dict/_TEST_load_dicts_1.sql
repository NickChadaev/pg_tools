  ...
  FROM dblink ('ccrm',
               $$
                 ...
                 FROM metamodel.m_entity_attribute mea
                  ...
                  LEFT JOIN uiconf.ui_element_mm_prop mp ON (mea.kd_entity = mp.kd_entity)
                          AND mea.kd_attribute = mp.kd_attribute
                          AND mp.id_element = (SELECT max(id_element) FROM uiconf.ui_element_mm_prop
...
where dct_dict.id_dict_parent is distinct from excluded.id_dict_parent
   ...
   or dct_dict.pr_delete <> excluded.pr_dele
   te;
