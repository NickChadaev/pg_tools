...
# FILE: load_gar_add_house_type.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_add_house_type (
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_add_house_type: "

# ------------------------------------------------------------------------------

class AddHouseTypeHandler( xml.sax.ContentHandler ):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "HOUSETYPE":
         ...
         self.shortname  = attributes.get("SHORTNAME",'')
         self.desc       = attributes.get("DESC",'')
         self.isactive   = attributes.get("ISACTIVE",'')
         self.updatedate = attributes.get("UPDATEDATE",'')
         self.startdate  = attributes.get("STARTDATE",'')
         self.enddate    = attributes.get("ENDDATE",'')
         #
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.name, self.shortname, self.desc,\
                  self.updatedate, self.startdate, self.enddate, self.isactive))              
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = AddHouseTypeHandler (self.conn_x, self.cur_x, 0)
