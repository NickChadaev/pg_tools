-- ----------------
-- 2015-08-20 Роман
-- Представление зависимостей  в базе
-- 2015-09-12 Nick Теперь название представления имеет стуктуру, принятую в проекте. 
-- ------------------------------------------------------------------------------------------------------------
SET search_path = db_info, public, pg_catalog;

DROP VIEW IF EXISTS db_info.v_dependency;
CREATE OR REPLACE VIEW db_info.v_dependency AS
WITH RECURSIVE preference AS (
  SELECT 10 AS max_depth -- максимальная глубина
    , 16384 AS min_oid -- только пользовательские объекты
...
, dependency_pair AS (
    ...
    AND (refobjid >= preference.min_oid OR refobjid = 2200) -- необходима public схема в качестве корневого узла
...
, dependency_hierarchy AS (
    ...
    AND NOT (child.objid = ANY(parent.dependency_chain)) -- Предотвращение зацикливания
)
SELECT * FROM dependency_hierarchy ORDER BY dependency_chain ;
 
COMMENT ON VIEW db_info.v_dependency IS 'Представление зависимостей  в базе';
--SELECT * FROM db_info.dependency