...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_rooms_params (
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_rooms_params: "

# ------------------------------------------------------------------------------

class RoomsParamsHandler( xml.sax.ContentHandler ):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "PARAM":
         ...
         #
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.object_id, self.change_id, self.change_id_end,\
                  self.type_id, self.value, self.update_date, self.start_date, self.end_date))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = RoomsParamsHandler (self.conn_x, self.cur_x, 0)
