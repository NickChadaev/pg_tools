...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_addr_obj: "
# ------------------------------------------------------------------------------

class AddrObjHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "OBJECT":
         ...
         self.object_name  = attributes.get("NAME",'')  
         self.type_name    = attributes.get("TYPENAME",'')  
         self.obj_level    = attributes.get("LEVEL",'')   
         ...
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.object_id, self.object_guid,\
                  self.change_id, self.object_name, self.type_name, self.obj_level,\
                      self.oper_type_id, self.prev_id, self.next_id, self.update_date,\
                          self.start_date, self.end_date, self.is_actual, self.is_active))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = AddrObjHandler (self.conn_x, self.cur_x, 0)
