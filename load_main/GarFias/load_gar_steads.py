...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_steads: "

# ------------------------------------------------------------------------------

class SteadsHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "STEAD":
         ...
         self.steads_number = attributes.get("NUMBER",'')
         ...
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.object_id, self.object_guid,\
                  self.change_id, self.steads_number, self.oper_type_id, self.prev_id,\
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = SteadsHandler (self.conn_x, self.cur_x, 0)
