/* ---------------------------------------------------------------------------------------------------------------------
        Входные параметры:
                1) p_nso_id    public.id_t      -- Идентификатор НСО
...
--------------------------------------------------------------------------------------------------------------------- */
DROP FUNCTION IF EXISTS nso_structure.nso_p_object_d (public.id_t,public.t_boolean);
CREATE OR REPLACE FUNCTION nso_structure.nso_p_object_d
(
        p_nso_id    public.id_t      -- Идентификатор НСО
...
RETURNS public.result_long_t
 ...
 $$
   ...
   BEGIN
     IF p_nso_id IS NOT NULL
     THEN
         SELECT nso_code, is_group_nso INTO _nso_code, _is_group
         FROM ONLY nso.nso_object WHERE nso_id = p_nso_id;
         
         c_MESS_OK = 'Успешно удален нормативно-справочный объект с кодом "' || _nso_code || '".';
         
         IF p_nso_id IS NOT NULL AND _nso_code IS NOT NULL
         THEN
             ...
             THEN
                  ...
                  IF _is_group
                  THEN
                       PERFORM nso_strucrure.nso_p_object_d (nso_id, p_node_flag) FROM ONLY nso.nso_object
                         WHERE parent_nso_id = p_nso_id;
                  END IF;
                  --
                  DELETE FROM ONLY nso.nso_abs WHERE rec_id IN
                          (SELECT rec_id FROM ONLY nso.nso_record WHERE nso_id = p_nso_id);
                  --
                  ...
                  -- Nick 2016-08-26
                  DELETE FROM ONLY nso.nso_record WHERE nso_id = p_nso_id;
                  --
                  DELETE FROM ONLY nso.nso_key_attr WHERE key_id IN
                          (SELECT key_id FROM ONLY nso.nso_key WHERE nso_id = p_nso_id);
                  --
                  DELETE FROM ONLY nso.nso_key         WHERE nso_id = p_nso_id;
                  DELETE FROM ONLY nso.nso_column_head WHERE nso_id = p_nso_id;
                  DELETE FROM ONLY nso.nso_object      WHERE nso_id = p_nso_id;
                  --
...
COMMENT ON FUNCTION nso_structure.nso_p_object_d (public.id_t, public.t_boolean)
  IS '193: Удаление Нормативно-Справочного Объекта, аргумент ID НСО
        Входные параметры:
                1) p_nso_id    public.id_t      -- Идентификатор НСО
                2) p_node_flag public.t_boolean -- Разрешение на удаление узлового элемента, по умолчанию FALSE (запрещено)
	Выходные параметры:
