DROP VIEW if EXISTS public.zz5;
CREATE OR REPLACE VIEW  public.zz5 AS
   WITH RECURSIVE aa1 (
                         id_addr_obj       
                        ,id_addr_parent 
                        --
...
-- SELECT * FROM public.zz5;  -- 26520   26515
...
--                                             ,UPPER(aa1.nm_addr_obj) 
--                                      --  ORDER BY aa1.change_id DESC  
--                     ) AS rn
                  
            FROM public.zz5 AS aa1 ;  -- 26514

SELECT * FROM gar_tmp_pcg_trans.f_xxx_adr_area_show_data() --   + 270

EXCEPT

SELECT * FROM gar_tmp_pcg_trans."_OLD_f_xxx_adr_area_show_data"() 
...
-- ================================================================================
       SELECT  
            ...
            FROM public.zz5 AS aa1 GROUP BY aa1.id_addr_parent, aa1.addr_obj_type, aa1.nm_addr_obj -- 26514
               ORDER BY 1 DESC;
			   
SELECT aa1.* FROM public.zz5 AS aa1 WHERE (aa1.nm_addr_obj IN 			   
...
SELECT aa1.* FROM public.zz5 AS aa1 WHERE (aa1.id_addr_obj IN 			   
( 84653
,75729
,84783
,75661
...
--
   WITH z AS (
     ...
     ---INNER JOIN  public.zz5 aa ON (aa.fias_guid = ah.parent_fias_guid);
