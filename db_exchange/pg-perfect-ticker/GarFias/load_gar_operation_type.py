...
# FILE: load_gar_operation_type.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_operation_type (
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_operation_type: "

# ------------------------------------------------------------------------------

class OperationTypeHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "OPERATIONTYPE":
         #
         self.oper_type_id   = attributes.get("ID",'')
         self.oper_type_name = attributes.get("NAME",'')
         self.short_name     = attributes.get("SHORTNAME",'')
         self.descr          = attributes.get("DESC",'')
         ...
         try:
              self.cur_x.execute (CALL_PROC, (self.oper_type_id, self.oper_type_name,\
                  self.short_name, self.descr, self.update_date,\
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = OperationTypeHandler (self.conn_x, self.cur_x, 0)
