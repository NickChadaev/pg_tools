...
# FILE: load_gar_norm_docs_types.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_norm_docs_types (
                    i_id          := (NULLIF (%s, ''))::bigint       
                   ,i_doc_name    := (NULLIF (%s, ''))::varchar(500)
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_norm_docs_types: "

# ------------------------------------------------------------------------------

class NormativeDocsTypesHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "NDOCTYPE":
         ...
         #
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.doc_name, self.start_date,\
                  self.end_date))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = NormativeDocsTypesHandler (self.conn_x, self.cur_x, 0)
