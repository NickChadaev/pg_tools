...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_param_type (
     i_id          := (NULLIF (%s, ''))::bigint
    ,i_type_name   := (NULLIF (%s, ''))::varchar(50)
    ,i_type_desc   := (NULLIF (%s, ''))::varchar(250)
    ,i_type_code   := (NULLIF (%s, ''))::varchar(250)
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_param_type: "

# ------------------------------------------------------------------------------

class ParamTypeHandler( xml.sax.ContentHandler ):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "PARAMTYPE":
         ...
         self.type_code   = attributes.get("CODE",'')
         ...
         #
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.type_name, self.type_desc, self.type_code,\
                  self.is_active, self.update_date, self.start_date, self.end_date))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = ParamTypeHandler (self.conn_x, self.cur_x, 0)
