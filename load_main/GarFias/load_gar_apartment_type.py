...
# FILE: load_gar_apartment_type.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_apartment_type (
                    i_id             := (NULLIF (%s, ''))::bigint
                   ,i_type_name      := (NULLIF (%s, ''))::varchar(100)
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_apartment_type: "

# ------------------------------------------------------------------------------

class ApartmentTypeHandler (xml.sax.ContentHandler):
   """
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = ApartmentTypeHandler (self.conn_x, self.cur_x, 0)
