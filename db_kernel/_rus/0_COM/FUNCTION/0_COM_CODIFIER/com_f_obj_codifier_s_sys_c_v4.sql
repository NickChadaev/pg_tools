DROP FUNCTION IF EXISTS com_codifier.com_f_obj_codifier_s_sys ( public.t_str60, public.t_timestamp, public.t_int, public.t_text ) CASCADE;
CREATE OR REPLACE FUNCTION com_codifier.com_f_obj_codifier_s_sys 
                        (  p_codif_code   public.t_str60            -- Код узла 
 ...
 RETURNS TABLE  (
  ...
  AS
           ...
           AS (
               WITH RECURSIVE cd (  codif_id
                          ...
                          SELECT 
                           ...
                           , c.date_from   -- 2015-05-30 Nick
                           , c.codif_uuid                    
                           , CAST (d.tree_d::int8[] || c.codif_id::int8 AS public.t_arr_id) -- на 9.4 это работало.
   ...
   $$
     STABLE 
     SECURITY INVOKER
     SET search_path=com_codifier, com, public
     LANGUAGE plpgsql ;

COMMENT ON FUNCTION com_codifier.com_f_obj_codifier_s_sys ( public.t_str60, public.t_timestamp, public.t_int, public.t_text ) 
   IS '45: Отображение кодификатора по уровням иерархии

   Аргументы:
      1)  p_codif_code   public.t_str60            -- Код узла 
...
-- select * from com_codifier.com_f_obj_codifier_s_sys ( p_codif_code := 'C_OBJ_TYPE', p_level_d  := 2);
-- select * from com_codifier.com_f_obj_codifier_s_sys ( p_codif_code := 'C_OBJ_TYPE', p_sort_hstore :=  '1 => "qty_d DESC"'::t_text);
 
-- select * from com_codifier.com_f_obj_codifier_s_sys ('C_PM');
-- select * from com_codifier.com_f_obj_codifier_s_sys ('C_CODIF_ROOT');
-- select * from com_codifier.com_f_obj_codifier_s_sys ('zggfd');

-- select * from com_codifier.com_f_obj_codifier_s_sys ( p_codif_id := 364, p_level_d  := 3, p_sort_hstore :=  '1 => "tree_d ASC"'::t_text);
