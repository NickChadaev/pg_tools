...
# FILE: load_gar_addr_obj_type.py
...
CALL_PROC = """CALL gar_fias_pcg_load.save_gar_addr_obj_type (
                    i_id             := (NULLIF (%s, ''))::bigint
                   ,i_type_level     := (NULLIF (%s, ''))::varchar(10)
                   ,i_type_name      := (NULLIF (%s, ''))::varchar(250)
                   ,i_type_shortname := (NULLIF (%s, ''))::varchar(50)
...
# 2022-05-25
BSIZE = 500
MESS_ABOUT = "gar_fias.as_addr_obj_type: "

# ------------------------------------------------------------------------------

class AddrObjTypeHandler (xml.sax.ContentHandler):
   """
   ...
   def startElement(self, tag, attributes):
      ...
      if tag == "ADDRESSOBJECTTYPE":
         ...
         self.type_level     = attributes.get("LEVEL",'')         
         ...
         try:
              self.cur_x.execute (CALL_PROC, (self.id, self.type_level, self.type_name,\
                  self.type_shortname, self.type_desc, self.update_date,\
                      self.start_date, self.end_date, self.is_active))
...
class MakeLoad ():
 ...
 def ToDo ( self, p_host_ip, p_port, p_db_name, p_user_name, p_gar_file_path):
    ...
    # -- Handler
    Handler = AddrObjTypeHandler (self.conn_x, self.cur_x, 0)
