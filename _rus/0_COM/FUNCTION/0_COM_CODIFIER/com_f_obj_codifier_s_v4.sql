/* --------------------------------------------------------------------------------
	...
	В случае, если явно указан "Код дочернего элемента",
	отображается только одна строка.
----------------------------------------------------------------*/
DROP FUNCTION IF EXISTS com_codifier.com_f_obj_codifier_s (public.t_str60, public.t_str60 ) CASCADE;
CREATE OR REPLACE FUNCTION com_codifier.com_f_obj_codifier_s (
                       p_codif_code_parent public.t_str60
                      ,p_codif_code_child  public.t_str60 DEFAULT NULL::public.t_str60
)
  RETURNS TABLE (
...
$$
  ...
  /* ========================================================================== */
   DECLARE
     ...
     _codif_code_child  public.t_str60 := utl.com_f_empty_string_to_null (btrim (upper (p_codif_code_child)));
  ...
  RETURN QUERY
          SELECT
           ...
           WHERE (b.codif_code = _codif_code_parent) AND 
                 ((_codif_code_child IS NULL) OR ((_codif_code_child IS NOT NULL) AND (_codif_code_child = a.codif_code)));
   END;
$$
  LANGUAGE plpgsql
...
COMMENT ON FUNCTION com_codifier.com_f_obj_codifier_s(public.t_str60, public.t_str60) IS '70: Отображение ветки кодификатора на один уровень вниз.
 
 Входные параметры:
      1)  p_codif_code_parent  public.t_str60		   -- Код родительского кодификатора
      2) ,p_codif_code_child   public.t_str60	= NULL -- Код дочернего элемента
...
-- -------------------------------------------------------------------------------------------
      ...
      SELECT * FROM com_codifier.com_f_obj_codifier_s (''C_ATTR_TYPE'',''T_STR60'');
...
-- 139|1|'0'|'C_OBJECT_LINK' |'Типы связей между объектами'                              |16|t
-- 147|1|'0'|'C_CD_TYPES'    |'Специфические типы компании КристалД'                     | 4|t
-- 160|1|'r'|'C_IND_TYPE'    |'Типы показателей'                                         |24|t
-- 182|1|'0'|'C_EXEC_TYPE'   |'Типизация исполнения'                                     | 2|t
-- 201|1|'0'|'C_AUTH_CONST'  |'Список констант для схемы AUTH'                           | 4|t
-- 233|1|'0'|'C_ACTION_TYPE' |'Перечень типов действий'                                  |60|t
-- 340|1|'0'|'C_RUBBISH_TYPE'|'Перечень видов и характеров, перенесённый из схемы PUBLIC'| 3|t
-- 459|1|'0'|'C_ACCOUNT_TYPE'|'Типы счетов'                                              | 3|t
-- 464|1|'0'|'C_APP_ROLE'    |'Прикладная роль'                                          | 1|t
-- -------------------------------------------------------------------------------------------
--     SELECT * FROM com_codifier.com_f_obj_codifier_s_sys('C_ATTR_TYPE');
--     SELECT * FROM com_codifier.com_f_obj_codifier_s ('C_ATTR_TYPE');
--     SELECT * FROM com_codifier.com_f_obj_codifier_s ('C_ATTR_TYPE','C_DOMEN_NODE');
--     SELECT * FROM com_codifier.com_f_obj_codifier_s (NULL); 
