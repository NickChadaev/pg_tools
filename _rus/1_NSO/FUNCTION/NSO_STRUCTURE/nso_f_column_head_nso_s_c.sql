DROP FUNCTION IF EXISTS nso_structure.nso_f_column_head_nso_s ( public.t_str60, public.t_boolean );
CREATE OR REPLACE FUNCTION nso_structure.nso_f_column_head_nso_s ( p_nso_code public.t_str60, p_priznak public.t_boolean = FALSE )
RETURNS TABLE ( 
...
AS
 $$
  ...
  DECLARE
    _nso_id  public.id_t;
  BEGIN
     _nso_id := (SELECT nso_id FROM ONLY nso.nso_object WHERE (nso_code = utl.com_f_empty_string_to_null(upper (btrim (p_nso_code)))));
                       ...
                       AS (
                            SELECT k.key_id
                             ...
                             SELECT
                                  ...
                                  --                             
                                  ,y.key_type_id::public.id_t  -- Код типа ключа    
 ...
 $$
    ...
    LANGUAGE plpgsql;
COMMENT ON FUNCTION nso_structure.nso_f_column_head_nso_s ( public.t_str60, public.t_boolean) IS '179: Построение списка колонок НСО. Аргумент Код НСО';

-- SELECT * FROM plpgsql_check_function_tb ('nso_structure.nso_f_column_head_nso_s ( public.t_str60, public.t_boolean )', security_warnings := true, fatal_errors := false);
-- 'nso_structure.nso_f_column_head_nso_s'|153|'RETURN QUERY'|'42883'|'operator does not exist: t_arr_id || id_t'|''|'No operator matches the given name and argument type(s). You might need to add explicit type casts.'
-- SELECT * FROM nso_structure.nso_f_column_head_nso_s ('SPR_RNTd'); 

-- SELECT * FROM nso.nso_key;

-- Пример выполнения:
--     SELECt * FROM nso_structure.nso_f_column_head_nso_s ('Cl_OKv'); 
--       SELECT * FROM nso.nso_f_record_s ( 14 );
